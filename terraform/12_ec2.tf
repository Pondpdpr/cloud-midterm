...
resource "aws_instance" "wp_server" {
    ...
    user_data = <<-EOF
                ...
                sudo wp core install --path=/var/www/html --title="Cloud" --allow-root --url=$WP_PUBLIC_IP --admin_user=$WP_ADMIN_USER --admin_password=$WP_ADMIN_PASS --admin_email="exmaple@example.com" --skip-email
                sleep 10
