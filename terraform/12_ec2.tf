...
resource "aws_instance" "wp_server" {
    ...
    user_data = <<-EOF
                ...
                sudo wp core install --path=/var/www/html --allow-root --url=$WP_PUBLIC_IP --title="Cloud" --admin_user=$WP_ADMIN_USER --admin_password=$WP_ADMIN_PASS --admin_email="example@example.com" --skip-email
                sleep 10
